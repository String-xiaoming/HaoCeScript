...
var SCRIPT_VERSION = "2026-04-15-js-bookshelf-workflow-v1";
...
var LOG_PATH = files.join(SCRIPT_DIR, "haoce_autojs.log");
...
var TEXT_BACK = "\u8fd4\u56de";
var TEXT_SEARCH = "\u641c\u7d22";
var TEXT_BOOKSHELF = "\u6211\u7684\u4e66\u67b6";
var TEXT_CHAPTER_UPDATE = "\u7ae0\u8282\u5b8c\u7ed3\u60c5\u51b5\u66f4\u65b0";
var TEXT_FINISHED = "\u5b8c\u7ed3";
var TEXT_COMPLETE = "\u5b8c\u6210";
...
function initializeLogFile() {
  ...
  try {
    if (typeof files.createWithDirs === "function") {
...
function readJson(path, fallbackValue) {
  try {
    ...
    if (content && content.charCodeAt && content.charCodeAt(0) === 0xfeff) {
...
function buildConfig(raw) {
  ...
  var pageTurnMinIntervalMs = parseRange(raw.page_turn.min_interval_ms, 192000, 258000);
  pageTurnMinIntervalMs[0] = Math.max(192000, pageTurnMinIntervalMs[0]);
  pageTurnMinIntervalMs[1] = Math.max(pageTurnMinIntervalMs[0], Math.max(258000, pageTurnMinIntervalMs[1]));
  ...
  return {
    ...
    pageTurn: {
      ...
      minIntervalMs: pageTurnMinIntervalMs
    },
    motion: {
      ...
      lateralChance: choose(raw.motion && raw.motion.lateral_chance, 0.05),
      lateralDistanceRatio: parseRange(raw.motion && raw.motion.lateral_distance_ratio, 0.035, 0.055),
      lateralDurationMs: parseRange(raw.motion && raw.motion.lateral_duration_ms, 150, 240),
      upwardChance: choose(raw.motion && raw.motion.upward_chance, 0.0),
      downwardChance: choose(raw.motion && raw.motion.downward_chance, 0.08),
      downwardDistanceRatio: parseRange(raw.motion && raw.motion.downward_distance_ratio, 0.018, 0.09),
      downwardDurationMs: parseRange(raw.motion && raw.motion.downward_duration_ms, 80, 180),
...
function HaoceReader(config) {
  ...
  this.pendingPageTurnIntervalMs = null;
...
HaoceReader.prototype.formatMsRange = function (rangeValue) {
  return rangeValue[0] === rangeValue[1] ?
    rangeValue[0] + "ms" :
    rangeValue[0] + "-" + rangeValue[1] + "ms";
};

HaoceReader.prototype.pageTurnIntervalTargetMs = function () {
  if (this.pendingPageTurnIntervalMs === null || this.pendingPageTurnIntervalMs === undefined) {
    this.pendingPageTurnIntervalMs = randomInt(
      this.config.pageTurn.minIntervalMs[0],
      this.config.pageTurn.minIntervalMs[1]
    );
    if (this.config.pageTurn.minIntervalMs[0] === this.config.pageTurn.minIntervalMs[1]) {
      log("page-turn interval target locked at " + this.pendingPageTurnIntervalMs + "ms");
    } else {
      log(
        "page-turn interval target selected " + this.pendingPageTurnIntervalMs +
        "ms (random in " + this.config.pageTurn.minIntervalMs[0] +
        "-" + this.config.pageTurn.minIntervalMs[1] + "ms)"
      );
    }
  }
  return this.pendingPageTurnIntervalMs;
};

HaoceReader.prototype.resetPageTurnIntervalTarget = function () {
  this.pendingPageTurnIntervalMs = null;
};

HaoceReader.prototype.buildDownwardReviewPlan = function (pauseMs) {
  ...
  distanceRatio = Math.max(lowDistance, Math.min(highDistance, distanceRatio * randomFloat(0.90, 1.08)));

  var distanceProgress = Math.max(0, Math.min(1, distanceRatio / Math.max(highDistance, 0.001)));
  var duration = Math.round(
    this.config.motion.downwardDurationMs[0] +
    (this.config.motion.downwardDurationMs[1] - this.config.motion.downwardDurationMs[0]) * distanceProgress
  );
  duration = Math.round(duration * randomFloat(0.92, 1.08));
  duration = Math.max(
    this.config.motion.downwardDurationMs[0],
    Math.min(this.config.motion.downwardDurationMs[1], duration)
  );
  ...
  var referencePauseMs = Math.max(
    1000,
    Math.floor((this.config.scroll.pauseMs[0] + this.config.scroll.pauseMs[1]) / 2)
  );
  var proportionalPauseMs = Math.round(
    referencePauseMs * distanceRatio / Math.max(highDistance, 0.001)
  );
  proportionalPauseMs = Math.round(proportionalPauseMs * randomFloat(0.94, 1.08));
  proportionalPauseMs = Math.max(1200, proportionalPauseMs);

  return {
    start: this.ratioToAbs(startRatio),
    end: this.ratioToAbs(endRatio),
    durationMs: duration,
    pauseRatio: pauseRatio,
    distanceRatio: distanceRatio,
    proportionalPauseMs: proportionalPauseMs
  };
};
...
HaoceReader.prototype.performLateralDrift = function () {
  var configuredDistancePx = Math.round(device.width * this.randomFloatBetween(this.config.motion.lateralDistanceRatio));
  var minDistancePx = Math.max(40, Math.round(device.width * 0.035));
  var maxDistancePx = Math.max(minDistancePx + 8, Math.round(device.width * 0.06));
  var distancePx = Math.max(minDistancePx, Math.min(maxDistancePx, configuredDistancePx));
  ...
  var centerY = randomFloat(0.48, 0.76);
  var halfDistance = distancePx / Math.max(device.width * 2, 1);
  var yDrift = randomFloat(-0.003, 0.003);
  ...
  var end = this.ratioToAbs([
    ...
    centerY + randomFloat(-0.002, 0.002)
  ]);
  log(
    "left drift plan: horizontal_distance=" + Math.abs(start[0] - end[0]) +
    "px, vertical_delta=" + Math.abs(start[1] - end[1]) + "px"
  );
  this.performGesturePath("left drift swipe", start, end, duration);
  return duration;
};

HaoceReader.prototype.performDownwardReview = function (plan) {
  log(
    "downward review swipe: pause_ratio=" + plan.pauseRatio.toFixed(2) +
    ", distance_ratio=" + plan.distanceRatio.toFixed(3) +
    ", proportional_pause=" + plan.proportionalPauseMs + "ms"
  );
...
HaoceReader.prototype.performPauseActions = function (label, pauseMs, options) {
  ...
  var chanceScale = Math.max(0, choose(options.chanceScale, 1.0));
  var compressDownwardPause = options.compressDownwardPause === true;
  var actions = [];
  if (allowLateral && Math.random() < Math.min(1, this.config.motion.lateralChance * chanceScale)) {
    actions.push("lateral");
  }
  if (allowDownward && Math.random() < Math.min(1, this.config.motion.downwardChance * chanceScale)) {
    actions.push("downward");
  }
  if (allowUpward && Math.random() < Math.min(1, this.config.motion.upwardChance * chanceScale)) {
    actions.push("upward");
  }
  ...
  var downwardPlan = action === "downward" ? this.buildDownwardReviewPlan(pauseMs) : null;
  var activePauseMs = pauseMs;
  if (downwardPlan && compressDownwardPause) {
    activePauseMs = Math.min(pauseMs, downwardPlan.proportionalPauseMs);
    if (activePauseMs !== pauseMs) {
      log(label + ": proportional downward pause " + activePauseMs + "ms (original " + pauseMs + "ms)");
    }
  }
  var remainingMs = activePauseMs;
  var waitMin = Math.min(remainingMs, Math.floor(activePauseMs * 0.2));
  var waitMax = Math.min(remainingMs, Math.max(waitMin, Math.floor(activePauseMs * 0.7)));
  var waitMs = 0;
  if (waitMax > 0) {
    waitMs = waitMin === waitMax ? waitMin : randomInt(waitMin, waitMax);
    sleepMs(waitMs);
  ...
  var usedMs = action === "lateral" ?
    ...
    this.performDownwardReview(downwardPlan || this.buildDownwardReviewPlan(pauseMs));
  ...
  log(
    label + ": action=" + action +
    ", pre_wait=" + waitMs + "ms, action_duration=" + usedMs +
    "ms, settle=" + settleMs + "ms, post_wait=" + remainingMs +
    "ms, total=" + activePauseMs + "ms"
  );
...
HaoceReader.prototype.enforcePageTurnInterval = function (lastPageTurnAt) {
  var low = this.config.pageTurn.minIntervalMs[0];
  var high = this.config.pageTurn.minIntervalMs[1];
  if (!lastPageTurnAt || high <= 0) {
    this.resetPageTurnIntervalTarget();
    return;
  }

  var minIntervalMs = this.pageTurnIntervalTargetMs();
  ...
  var chunkLow = Math.max(30000, this.config.scroll.pauseMs[0] * 2);
  var chunkHigh = Math.max(chunkLow, Math.max(55000, this.config.scroll.pauseMs[1] * 3));
  ...
  while (remainingMs > 0) {
    ...
    this.performPauseActions("page-turn interval pause", chunkMs, {
      ...
      allowDownward: true,
      allowUpward: false,
      forceRandom: remainingMs <= chunkMs,
      chanceScale: 0.45,
      compressDownwardPause: false
    });
...
HaoceReader.prototype.performPageTurn = function () {
  ...
  var duration = Math.max(
    120,
    this.randomizedDuration(this.config.pageTurn.durationMs, this.config.pageTurn.durationJitterMs)
  );
...
HaoceReader.prototype.run = function (skipPrepare, maxPageTurnsOverride) {
  ...
  log(
    ...
    "min_interval=" + this.formatMsRange(this.config.pageTurn.minIntervalMs)
  );
...
HaoceReader.prototype.safeClickable = function (node) {
  try {
...
HaoceReader.prototype.pageName = function () {
  if (text(TEXT_CHAPTER_UPDATE).exists() || textContains(TEXT_CONTINUE).exists() || text(TEXT_REPORT).exists()) {
    return "detail";
  }
  if (text(TEXT_BOOKSHELF).exists() && desc(TEXT_BACK).exists()) {
    return "bookshelf";
  }
  if (text(TEXT_RECENT).exists() || text(TEXT_HOME).exists()) {
    return "home";
...
HaoceReader.prototype.tapNodeCenter = function (bounds, label) {
  if (!bounds) {
    return false;
  }
  var point = rectCenter(bounds);
  if (label) {
    log(label + ": tapping " + point[0] + "," + point[1]);
  }
  singleTap(point[0], point[1]);
  return true;
};

HaoceReader.prototype.tapTopBackIfPresent = function () {
  var node = desc(TEXT_BACK).findOne(500);
  if (!node) {
    return false;
  }
  var bounds = this.safeBounds(node);
  ...
  return this.tapNodeCenter(bounds, "top back");
};

HaoceReader.prototype.waitForPage = function (targetPage, timeoutMs) {
  var deadline = new Date().getTime() + choose(timeoutMs, 6000);
  while (new Date().getTime() < deadline) {
    if (this.pageName() === targetPage) {
      return true;
    }
    sleepMs(300);
  }
  return this.pageName() === targetPage;
};

HaoceReader.prototype.ensureHomePage = function () {
  if (this.pageName() === "home") {
    return true;
  }
  ...
  for (var attempt = 0; attempt < 8; attempt += 1) {
    ...
    if (pageName === "bookshelf" || pageName === "detail") {
      if (!this.tapTopBackIfPresent()) {
        back();
      }
    } else {
      back();
    }
    sleepMs(Math.max(this.config.navigation.prepareWaitMs, 1400));
  }
...
HaoceReader.prototype.findBottomBookshelfEntry = function () {
  if (this.pageName() !== "home") {
    return null;
  }
  var nodes = this.allNodes();
  ...
  for (var i = 0; i < nodes.length; i += 1) {
    var textValue = this.safeText(nodes[i]);
    var bounds = this.safeBounds(nodes[i]);
    if (textValue !== TEXT_BOOKSHELF || !bounds || rectArea(bounds) <= 0) {
      continue;
    }
    if (bounds.top < 1850) {
      continue;
    }
    if (!bestBounds || bounds.top > bestBounds.top) {
      bestBounds = bounds;
    }
  }
  return bestBounds;
...
HaoceReader.prototype.openBookshelfPage = function () {
  if (this.pageName() === "bookshelf") {
    return true;
  }
  if (!this.ensureHomePage()) {
    return false;
  }

  for (var attempt = 0; attempt < 4; attempt += 1) {
    var bounds = this.findBottomBookshelfEntry();
    if (!bounds) {
      sleepMs(500);
      continue;
    }
    ...
    var iconPoint = [
      point[0],
      Math.max(0, bounds.top - 85)
    ];
    log("opening bookshelf shortcut at " + iconPoint[0] + "," + iconPoint[1] + " (attempt " + (attempt + 1) + ")");
    singleTap(iconPoint[0], iconPoint[1]);
    sleepMs(Math.max(this.config.navigation.prepareWaitMs, 1500));
    if (this.pageName() === "bookshelf") {
      return true;
    }

    log("bookshelf icon point did not open the page, trying text center");
    singleTap(point[0], point[1]);
    sleepMs(Math.max(this.config.navigation.prepareWaitMs, 1500));
    if (this.pageName() === "bookshelf") {
      return true;
  ...
  return this.pageName() === "bookshelf";
};

HaoceReader.prototype.performBookshelfScroll = function () {
  this.performDirectSwipe(
    "bookshelf scroll swipe",
    this.ratioToAbs([0.50, 0.82]),
    this.ratioToAbs([0.50, 0.56]),
    randomInt(380, 520)
  );
};

HaoceReader.prototype.performBookshelfBackScroll = function () {
  this.performDirectSwipe(
    "bookshelf reverse swipe",
    this.ratioToAbs([0.50, 0.44]),
    this.ratioToAbs([0.50, 0.72]),
    randomInt(380, 520)
  );
};

HaoceReader.prototype.listVisibleBookshelfCards = function () {
  if (this.pageName() !== "bookshelf") {
    return [];
  }
  ...
  for (var i = 0; i < nodes.length; i += 1) {
    var metricText = this.safeText(nodes[i]);
    var metricBounds = this.safeBounds(nodes[i]);
    var metricPercent = parsePercent(metricText);
    if (metricPercent === null || !metricBounds || rectArea(metricBounds) <= 0) {
      continue;
    }
    percentNodes.push({
      text: metricText,
  ...
  for (var j = 0; j < nodes.length; j += 1) {
    var node = nodes[j];
    var bounds = this.safeBounds(node);
    var title = normalizeTitle(this.safeDesc(node));
    if (!title || !bounds || rectArea(bounds) <= 120000) {
      continue;
    }
    if (!this.safeClickable(node) || bounds.left > 40 || bounds.top < 700 || rectWidth(bounds) < 900 || rectHeight(bounds) < 120) {
      continue;
    }
    if (seen[title]) {
      continue;
    ...
    for (var k = 0; k < percentNodes.length; k += 1) {
      var candidate = percentNodes[k];
      if (candidate.bounds.top < bounds.top + 60 || candidate.bounds.top > bounds.bottom + 12) {
        continue;
      }
      if (candidate.bounds.left < 140 || candidate.bounds.left > 640) {
        continue;
      }
      var distance = Math.abs(candidate.bounds.top - bounds.top);
    ...
    cards.push({
      title: title,
      bounds: bounds,
      progressText: bestMetric ? bestMetric.text : null,
      progressPercent: bestMetric ? bestMetric.percent : null
    });
  }
...
HaoceReader.prototype.findVisibleBookshelfCardByTitle = function (title) {
  var normalizedTitle = normalizeTitle(title);
  var cards = this.listVisibleBookshelfCards();
  for (var i = 0; i < cards.length; i += 1) {
    if (normalizeTitle(cards[i].title) === normalizedTitle) {
      return cards[i];
    }
  }
  return null;
...
HaoceReader.prototype.chooseBookshelfTarget = function () {
  if (!this.openBookshelfPage()) {
    return null;
  }
  ...
  for (var attempt = 0; attempt < 18; attempt += 1) {
    ...
    for (var cardIndex = 0; cardIndex < cards.length; cardIndex += 1) {
      titles.push(cards[cardIndex].title + ":" + cards[cardIndex].progressText);
    }
    log("bookshelf scan #" + (attempt + 1) + ": " + titles.join(" | "));

    var pageNewCount = 0;
    for (var i = 0; i < cards.length; i += 1) {
      var card = cards[i];
      var normalizedTitle = normalizeTitle(card.title);
      ...
      if (typeof card.progressPercent === "number" && card.progressPercent >= 99.9) {
        if (!lastUnfinished) {
          log("first completed bookshelf card has no preceding unfinished card yet: " + card.title);
          return null;
        }
        log("first completed bookshelf card: " + card.title + " (" + card.progressText + ")");
        log("target previous unfinished bookshelf card: " + lastUnfinished.title + " (" + lastUnfinished.progressText + ")");

        var visibleTarget = this.findVisibleBookshelfCardByTitle(lastUnfinished.title);
        if (visibleTarget) {
          return visibleTarget;
        }

        for (var retry = 0; retry < 3; retry += 1) {
          this.performBookshelfBackScroll();
          sleepMs(Math.max(this.config.navigation.prepareWaitMs, 1200));
          visibleTarget = this.findVisibleBookshelfCardByTitle(lastUnfinished.title);
          if (visibleTarget) {
            return visibleTarget;
          }
        }
    ...
    var signature = titles.join("||");
    if (signature === lastSignature || pageNewCount === 0) {
      stagnantRounds += 1;
    } else {
      stagnantRounds = 0;
    ...
    this.performBookshelfScroll();
    sleepMs(Math.max(this.config.navigation.prepareWaitMs, 1200));
  }
  return null;
};

HaoceReader.prototype.openBookshelfCard = function (card) {
  if (!card || !card.bounds) {
    return false;
  }

  var attempts = [
    rectCenter(card.bounds),
    [Math.floor((card.bounds.left + card.bounds.right) / 2), Math.min(device.height - 1, card.bounds.top + 72)]
  ];

  for (var i = 0; i < attempts.length; i += 1) {
    var point = attempts[i];
    log("opening bookshelf card: " + card.title + " at " + point[0] + "," + point[1] + " (attempt " + (i + 1) + ")");
    singleTap(point[0], point[1]);
    sleepMs(Math.max(this.config.navigation.prepareWaitMs, 1600));
    if (this.pageName() === "detail") {
      return true;
    }
  }
  return this.pageName() === "detail";
};

HaoceReader.prototype.findUpdateHeaderBounds = function () {
  var node = text(TEXT_CHAPTER_UPDATE).findOne(500);
  if (!node) {
    return null;
  }
  return this.safeBounds(node);
};

HaoceReader.prototype.scrollDetailToTop = function () {
  for (var attempt = 0; attempt < 6; attempt += 1) {
    var headerBounds = this.findUpdateHeaderBounds();
    if (headerBounds && headerBounds.top >= 820 && headerBounds.top <= 960) {
      return true;
    }
    this.performDirectSwipe(
      "detail return-top swipe",
      this.ratioToAbs([0.50, 0.42]),
      this.ratioToAbs([0.50, 0.80]),
      randomInt(380, 520)
    );
    sleepMs(Math.max(this.config.navigation.prepareWaitMs, 1200));
  }
  return this.findUpdateHeaderBounds() !== null;
};

HaoceReader.prototype.performDetailChapterScroll = function () {
  this.performDirectSwipe(
    "detail chapter swipe",
    this.ratioToAbs([0.50, 0.82]),
    this.ratioToAbs([0.50, 0.52]),
    randomInt(380, 520)
  );
};

HaoceReader.prototype.detailOverallProgress = function () {
  var nodes = this.allNodes();
  var bestValue = null;
  var bestTop = null;
  for (var i = 0; i < nodes.length; i += 1) {
    var textValue = this.safeText(nodes[i]);
    var bounds = this.safeBounds(nodes[i]);
    var percent = parsePercent(textValue);
    if (percent === null || !bounds || rectArea(bounds) <= 0) {
      continue;
    }
    if (bounds.top < 420 || bounds.top > 620 || bounds.left > 620) {
      continue;
    }
    if (bestTop === null || bounds.top < bestTop) {
      bestTop = bounds.top;
...
HaoceReader.prototype.detailLooksCompleted = function () {
  if (text(TEXT_COMPLETE).exists() || textContains(TEXT_COMPLETE).exists()) {
    return true;
  }
  var progress = this.detailOverallProgress();
  return typeof progress === "number" && progress >= 99.9;
};

HaoceReader.prototype.listVisibleChapterRows = function () {
  var headerBounds = this.findUpdateHeaderBounds();
  if (!headerBounds) {
    return [];
  }
  ...
  var leftNodes = [];
  var rightNodes = [];
  var ignoredLeftTexts = {
    "\u9605\u8bfb\u8017\u65f6": true,
    "\u9605\u8bfb\u8fdb\u5ea6": true,
    "\u9605\u8bfb\u901f\u5ea6": true,
    "\u9605\u8bfb\u5b57\u6570": true,
    TEXT_CHAPTER_UPDATE: true
  };
  ...
  for (var i = 0; i < nodes.length; i += 1) {
    ...
    if (bounds.top <= headerBounds.bottom + 30 || bounds.top >= device.height - 30) {
      continue;
    }

    if (bounds.left >= 850 && (textValue === TEXT_FINISHED || textValue.indexOf("%") >= 0 || textValue.indexOf(TEXT_COMPLETE) >= 0)) {
      rightNodes.push({
        text: textValue,
        bounds: bounds
      });
    ...
    if (bounds.left <= 120 && !ignoredLeftTexts[textValue]) {
      leftNodes.push({
        text: textValue,
        bounds: bounds
      });
  ...
  var rows = [];
  for (var j = 0; j < rightNodes.length; j += 1) {
    var rightNode = rightNodes[j];
    var bestLeft = null;
    var bestDelta = Number.MAX_SAFE_INTEGER;
    for (var k = 0; k < leftNodes.length; k += 1) {
      var leftNode = leftNodes[k];
      var delta = Math.abs(rectCenter(leftNode.bounds)[1] - rectCenter(rightNode.bounds)[1]);
      if (delta > 40) {
        continue;
      }
      if (delta < bestDelta) {
    ...
    rows.push({
      title: bestLeft.text,
      statusText: rightNode.text,
      statusPercent: parsePercent(rightNode.text),
      bounds: {
        left: bestLeft.bounds.left,
        top: Math.min(bestLeft.bounds.top, rightNode.bounds.top) - 12,
        right: rightNode.bounds.right,
        bottom: Math.max(bestLeft.bounds.bottom, rightNode.bounds.bottom) + 12
      }
    });
  }
...
HaoceReader.prototype.findFirstUnfinishedChapter = function () {
  var lastSignature = null;
  var stagnantRounds = 0;

  for (var attempt = 0; attempt < 18; attempt += 1) {
    var rows = this.listVisibleChapterRows();
    if (rows.length) {
      var parts = [];
      for (var i = 0; i < rows.length; i += 1) {
        parts.push(rows[i].title + ":" + rows[i].statusText);
        if (typeof rows[i].statusPercent === "number" && rows[i].statusPercent < 99.9) {
          log("first unfinished chapter found: " + rows[i].title + " -> " + rows[i].statusText);
          return rows[i];
        }
      }
      log("chapter scan #" + (attempt + 1) + ": " + parts.join(" | "));
    ...
    this.performDetailChapterScroll();
    sleepMs(Math.max(this.config.navigation.prepareWaitMs, 1200));
  }
  return null;
};

HaoceReader.prototype.openChapterRow = function (row) {
  if (!row || !row.bounds) {
    return false;
  }

  var tapPoints = [
    rectCenter(row.bounds),
    [Math.min(device.width - 1, row.bounds.right - 80), Math.floor((row.bounds.top + row.bounds.bottom) / 2)]
  ];

  for (var i = 0; i < tapPoints.length; i += 1) {
    var point = tapPoints[i];
    log("opening chapter row: " + row.title + " (" + row.statusText + ") at " + point[0] + "," + point[1]);
    singleTap(point[0], point[1]);
    sleepMs(Math.max(this.config.navigation.prepareWaitMs, 1600));
    if (this.pageName() !== "detail" && this.pageName() !== "bookshelf" && this.pageName() !== "home") {
      return true;
    }
  }
  return this.pageName() !== "detail" && this.pageName() !== "bookshelf" && this.pageName() !== "home";
};

HaoceReader.prototype.waitBottomDwell = function (chapterStartedAt, targetMs) {
  while (true) {
    var remainingMs = targetMs - (new Date().getTime() - chapterStartedAt);
    if (remainingMs <= 0) {
      return;
    }
    var chunkMs = Math.min(remainingMs, randomInt(15000, 28000));
    log("bottom dwell: waiting " + remainingMs + "ms more before chapter completion swipe");
    this.performPauseActions("bottom dwell", chunkMs, {
      allowLateral: true,
      allowDownward: false,
      allowUpward: false,
      forceRandom: false,
...
HaoceReader.prototype.performChapterCompletionSwipe = function () {
  for (var attempt = 0; attempt < 2; attempt += 1) {
    ...
    this.recycleImage(before);
    this.recycleImage(after);
    log("chapter completion swipe check: mean_diff=" + metrics.meanDiff.toFixed(3) + ", changed_ratio=" + metrics.changedRatio.toFixed(4));
    if (this.pageTurnConfirmed(metrics)) {
      return true;
    }
    sleepMs(1200);
  }
...
HaoceReader.prototype.readCurrentChapterTimed = function () {
  var chapterStartedAt = new Date().getTime();
  this.resetPageTurnIntervalTarget();
  var targetMs = this.pageTurnIntervalTargetMs();
  var stuckCount = 0;
  var iteration = 0;

  log("chapter reader loop started");
  while (true) {
    iteration += 1;
    var before = this.capture();
    this.performScroll();
    ...
    log("chapter scroll #" + iteration + ": mean_diff=" + metrics.meanDiff.toFixed(3) + ", changed_ratio=" + metrics.changedRatio.toFixed(4));

    if (this.looksStuck(metrics)) {
      stuckCount += 1;
      log("chapter bottom-like state " + stuckCount + "/" + this.config.analysis.bottomConfirmations);
      if (stuckCount < this.config.analysis.bottomConfirmations) {
        sleepMs(this.config.runtime.loopSleepMs);
        continue;
      }

      this.waitBottomDwell(chapterStartedAt, targetMs);
      if (!this.performChapterCompletionSwipe()) {
        log("chapter completion swipe was not confirmed");
        return false;
      }
      return true;
    }
...
HaoceReader.prototype.backToDetailPage = function () {
  for (var attempt = 0; attempt < 6; attempt += 1) {
    if (this.pageName() === "detail") {
      return true;
    }
...
HaoceReader.prototype.returnToHomeAfterBook = function () {
  for (var attempt = 0; attempt < 8; attempt += 1) {
    var pageName = this.pageName();
    if (pageName === "home") {
      return true;
    }
    if (pageName === "detail" || pageName === "bookshelf") {
      if (!this.tapTopBackIfPresent()) {
        back();
      }
    } else {
...
HaoceReader.prototype.processCurrentBook = function () {
  for (var round = 0; round < 40; round += 1) {
    if (!this.scrollDetailToTop()) {
      log("failed to return detail page to the update header");
      return "stop";
    }
    this.tapDetailUpdateIfPresent();
    sleepMs(Math.max(this.config.navigation.prepareWaitMs, 1200));

    if (this.detailLooksCompleted()) {
      log("book detail page already shows completion");
      return "completed";
    }

    var chapterRow = this.findFirstUnfinishedChapter();
    if (!chapterRow) {
      if (this.detailLooksCompleted()) {
        log("no unfinished chapter remained after update scan");
        return "completed";
      }
      log("failed to find any unfinished chapter row on the detail page");
      return "stop";
    }

    if (!this.openChapterRow(chapterRow)) {
      log("failed to enter the unfinished chapter");
      return "stop";
    }
    if (!this.readCurrentChapterTimed()) {
      return "stop";
    }
    if (!this.backToDetailPage()) {
      log("failed to return from chapter reader to the detail page");
      return "stop";
    }
  }
  log("book chapter loop exceeded the safety limit");
  return "stop";
};

HaoceReader.prototype.runBookshelfWorkflow = function () {
  if (this.config.navigation.launchApp) {
    this.launchApp();
  }
  if (!this.ensureHomePage()) {
    log("failed to reach the home page");
    return 2;
  }
  ...
  while (true) {
    cycle += 1;
    log("workflow cycle #" + cycle + " started");
    this.refreshRecentHome();

    var targetBook = this.chooseBookshelfTarget();
    if (!targetBook) {
      log("no target unfinished book before the first completed bookshelf card");
      return 0;
    }
    this.currentBook = targetBook;
    log("selected workflow book: " + this.describeBook(targetBook));

    if (!this.openBookshelfCard(targetBook)) {
      log("failed to open the selected bookshelf book");
      return 2;
    }

    var result = this.processCurrentBook();
    if (result !== "completed") {
      return 2;
    }

    if (!this.returnToHomeAfterBook()) {
      log("failed to return to the home page after completing the book");
      return 2;
    }
    this.refreshRecentHome();
  }
...
function main() {
  ...
  var exitCode = reader.runBookshelfWorkflow();
